...
function pushAllStreams() {
  ...
  api.pushSensorValue('roll', degToRad(Math.sin(t * 0.4) * 3 + Math.sin(t * 1.1) * 1.5 + randomVariation(0, 0.3)));
...
function generateLegacyPacket() {
  ...
  const heelAngle = Math.sin(t2 * 0.4) * 3 + Math.sin(t2 * 1.1) * 1.5 + randomVariation(0, 0.3);
