...
const [storageStats, setStorageStats] = useState<StorageStats | null>(null);
  ...
  const tabs: { id: SettingsTab; label: string; icon: JSX.Element }[] = [
    ...
    // Display tab — always shown (night mode applies to every device); the
    // Pi-only hardware controls inside it are gated on hasAgent.
    {
          ...
          <div key={group.key} style={{ marginBottom: theme.space.lg }}>
            ...
            {(group.key === 'depth' || group.key === 'heritage' || group.key === 'seabed') && (
              <div style={{
                ...
                {t(`downloads.group_${group.key}_note`)}
              </div>
            )}
  ...
  const renderActiveTab = () => {
    switch (activeTab) {
      ...
      case 'clients':
        return <ClientsTab />;
      case 'display':
        return <DisplayTab hasAgent={hasAgent} />;
      case 'advanced':
